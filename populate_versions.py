...
def generate_keys_versions():
    ...
    # Read simulator keys separately
    sim_file = versions_dir / 'version-sim.txt' if versions_dir.exists() else None
    simulator_keys = set()
    if sim_file and sim_file.exists():
        sort_version_file(sim_file)
        simulator_keys = read_version_file(sim_file)
        print(f"Reading {sim_file}: {len(simulator_keys)} simulator-only keys (sorted)\n")
    ...
    # Mark simulator keys first
    simulator_only = unmapped_keys & simulator_keys
    if simulator_only:
        print(f"Found {len(simulator_only)} simulator-only keys")
        for hash_str in simulator_only:
            version_map[hash_str] = "sim"

    # Remaining unmapped keys are truly unknown
    truly_unknown = unmapped_keys - simulator_keys
    if truly_unknown:
        print(f"Found {len(truly_unknown)} mapped keys not in any version file (marking as unknown)")
        for hash_str in truly_unknown:
    ...
    with output_file.open('w') as f:
        ...
        f.write('- "X.Y": First appeared in iOS X.Y\n')
        f.write('- "sim": Simulator-only key (not found in any physical iOS version)\n')
        f.write('- "unknown": Not found in any tracked version\n')
        f.write('Removed keys are tracked in KEY_IOS_REMOVED with the version they were removed.\n')
        f.write('Reintroduced keys are tracked in KEY_IOS_REINTRODUCED with versions they came back.\n')
        f.write('"""\n\n')
